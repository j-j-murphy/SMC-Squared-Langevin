...
class smc_diagnostics_final_output_only(smc_no_diagnostics):
    def __init__(self, model, proposal, l_kernel, step_size, seed):
        ...
        self.fpath = Path(f"outputs/{model}", f"{np.round(step_size, 5)}", f"{l_kernel}", f"{proposal}", f"seed_{int(seed)}")
    ...
    def save_final_info(self, mean, mean_rc, var, var_rc, Neff, resampling_points, N, runtime_iterations):
        ...
        mean = np.array(mean)
        mean_rc = np.array(mean_rc)
        var = np.array(var)
        var_rc = np.array(var_rc)
        resampling_points = np.array(resampling_points)
        runtime_iterations = np.array(runtime_iterations)
