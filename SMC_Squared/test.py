...
parser.add_argument('-start', '--start_step_size', type=float, default=0.4)
parser.add_argument('-num', '--num_steps', type=int, default=1)
...
class Target_PF():
    ...
    def run_particleFilter(self, thetas, rngs):
        #thetas_ = torch.tensor(thetas, requires_grad=True)
        logprob = torch.distributions.MultivariateNormal(torch.tensor([0,0.2]), torch.eye(2)).log_prob(thetas)
        return logprob
...
class Q0(Q0_Base):
    ...
    def __init__(self):
        ...
        self.uni_2_pdf = Uniform_PDF(loc=0, scale=3)

    def logpdf(self, x):
        return self.uni_1_pdf.logpdf(x[0]) + self.uni_2_pdf.logpdf(x[1])# +self.uni_2_pdf.logpdf(x[2])

    def rvs(self, size, rngs):
        return np.array([rngs.randomUniform(-1, 1)[0], rngs.randomUniform(0, 3)[0]])#, rngs.randomUniform(0, 5)[0]])
