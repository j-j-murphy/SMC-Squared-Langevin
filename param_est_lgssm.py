...
class Q0(Q0_Base):
    def __init__(self):
        ...
        self.uni_3_pdf = Uniform_PDF(loc=0, scale=2) #2
...
D = 3
P=500
model = f"lgssm_N_{N}_K_{K}_D_{D}_P_{P}_fix_v"
...
first_order_ss = np.linspace(0.01, 0.11, 21)
rw_ss = np.linspace(0.1, 1.1, 21)
# first_order_ss = [0.065]
# sec_order_ss = [1.6]
# rw_ss = [0.1]
prop_step_sizes=  {'second_order': sec_order_ss}#'rw': rw_ss, 'first_order':first_order_ss}#'second_order': sec_order_ss}#'rw': rw_ss, 'first_order':first_order_ss}#'}#}#}## , #
    ...
    for l_kernel in l_kernels:
        for seed in seeds:
            ...
            for step_size in step_sizes:
                ...
                q = Q(step_size, proposal, ss_2nd_1st=0.065)
